...
namespace Microsoft.CodeAnalysis.ExpressionEvaluator
{
    internal sealed class NativeViewExpansion : Expansion
    {
            ...
            EvalResultDataItem parent)
        {
            ...
            var formatString = IntPtr.Size == 4 ? "(IUnknown*)0x{0:X8}" : "(IUnknown*)0x{0:X16}";
            var name = string.Format(formatString, comObject.NativeComPointer);
            var fullName = string.Concat("{C++}", name);
